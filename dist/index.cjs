...
var require_cookies2 = __commonJS({
  "node_modules/nodemailer/lib/fetch/cookies.js"(exports, module2) {
    ...
    var Cookies = class {
      ...
      isExpired(cookie2) {
        return cookie2.expires && cookie2.expires < new Date() || !cookie2.value;
      }
      getPath(pathname) {
    ...
    function createDefaultLogger(levels) {
      ...
      let print = (level, entry, message, ...args) => {
        ...
        if (entry) {
          ...
          if (entry.sid) {
            prefix2 = "[" + entry.sid + "] " + prefix2;
          }
          if (entry.cid) {
            prefix2 = "[#" + entry.cid + "] " + prefix2;
          }
        }
        message = util.format(message, ...args);
        message.split(/\r?\n/).forEach((line) => {
          console.log("[%s] %s %s", new Date().toISOString().substr(0, 19).replace(/T/, " "), levelNames.get(level), prefix2 + line);
        });
      };
      let logger = {};
      levels.forEach((level) => {
    ...
    var Mail = class extends EventEmitter {
      ...
      _convertDataImages(mail, callback) {
        ...
        mail.resolveContent(mail.data, "html", (err, html) => {
          ...
          let cidCounter = 0;
          html = (html || "").toString().replace(/(<img\b[^>]* src\s*=[\s"']*)(data:([^;]+);[^"'>\s]+)/gi, (match, prefix2, dataUri, mimeType) => {
